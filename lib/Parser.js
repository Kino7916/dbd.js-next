...
function parse(input) {
    ...
    function delimited(start, stop, separator, parser, skip = skip_punc, check = is_punc) {
        let a = [], first = true;
        if (start)
            skip(start);
        while (!input.eof()) {
            ...
            if (first)
                ...
                separator ? check(separator) ? skip(separator) : null : null;
            if (check(stop))
                break;
            a.push(parser());
        }
    ...
    function parse_if() {
        ...
        if (args.length === 0)
            throw new Error(`Expected condition in if`);
